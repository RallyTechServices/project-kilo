(function () {
    ...
    Ext.define("Rally.apps.charts.settings.PortfolioDatePicker", {
        ...
        _setDefaultValues: function () {
            if (this._dateGroupHasNoSelection(this.startDateGroup) && this.actualStartDate && !this.actualStartDate.disabled) {
                this._selectRadio(this.startDateGroup, "actualstartdate");
            }

            if (this._dateGroupHasNoSelection(this.endDateGroup)) {
                if (this.actualEndDate && !this.actualEndDate.disabled) {
        ...
        _updateRadioLabel: function (radioComponent, date) {
            ...
            if (formattedDate) {
                radioComponent && radioComponent.enable();
                newLabelValue += " (" + formattedDate + ")";
            }
            else {
                radioComponent && radioComponent.disable();
            ...
            radioComponent && radioComponent.boxLabelEl.setHTML(newLabelValue);
        },

        _isActualDateRadioField: function (radioComponent) {
            return radioComponent && radioComponent.getId().indexOf("actual") > -1;
        }
    });
}());